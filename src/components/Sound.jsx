...
export default function Sound({src}) {
  ...
  const wasPlayingBeforeBlur = useRef(false);
   const colorTransition = { transition: "fill 300ms ease-in-out" };
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const darkNavElements = document.querySelectorAll('[data-dark-nav]');
      
      let isInDarkSection = false;
      
      darkNavElements.forEach((element) => {
        const rect = element.getBoundingClientRect();
        // Check if element is in viewport (specifically at the top where nav would be)
        if (rect.top <= 100 && rect.bottom >= 0) {
          isInDarkSection = true;
        }
      });
      
      setIsDarkTheme(isInDarkSection);
    };
  ...
  const fillColor = isDarkTheme ? "#A20601" : "#E7CB68";
  const borderColor = isDarkTheme ? "border-[#A20601]" : "border-[#E7CB68]";
  const hoverBgColor = isDarkTheme ? "hover:bg-[#A20601]/10" : "hover:bg-[#E7CB68]/10";
  const hoverShadowColor = isDarkTheme ? "hover:shadow-[#A20601]/20" : "hover:shadow-[#E7CB68]/20";
  const bgColor = isDarkTheme ? "bg-[#A20601]/5" : "bg-[#E7CB68]/5";
  const barBgColor = isDarkTheme ? "bg-[#A20601]" : "bg-[#E7CB68]";

  return (
    <div className="fixed  inset-0 px-[2vw] max-md:px-[4vw] max-md:py-[5vw] py-[1vw] w-full z-999 h-fit flex items-center justify-between">
      <div className="w-[8vw] max-md:w-[18vw] h-[8vw] max-md:h-[18vw]">
        <svg viewBox="0 0 300 300" className="w-full h-full">
          <path
            ...
            fill={fillColor}  style={colorTransition}
          />
          <path
      ...
      aria-label="Toggle Sound"
        ...
        className={`cursor-pointer border ${borderColor} rounded-full p-2 
          ...
          hover:scale-110 ${hoverBgColor} hover:shadow-lg ${hoverShadowColor}
          active:scale-95
          ${isPlaying ? bgColor : ""}
        `}
      >
        <div className="flex items-center justify-center gap-[3px] w-6 h-6">
          {[...Array(4)].map((_, index) => (
            <div
              ...
              className={`w-[2px] h-full ${barBgColor} rounded-full origin-center`}
